...
public:
    ...
    {
        return new npc_thugAI (pCreature);
    }
    ...
    struct npc_thugAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
            ...
            if(pWho->ToPlayer()->GetQuestStatus(QUEST_LOUS_PARTING_THOUGHTS) == QUEST_STATUS_INCOMPLETE)
            {
                if (pWho->IsWithinDistInMap(me, 10.0f) && !bSummoned)
                {
            ...
            if(SummonTimer < diff)
            {
                if(bSummoned)
                {
                    ...
                    if(Creature* Thug4 = me->GetCreature(*me, Thug4GUID))
                    {
                        switch(Phase)
                        {
                            case 0: Thug1->MonsterSay("Did you... did you meet her?",0,0); SummonTimer = 3500; Phase++; break;
                            case 1: Thug2->MonsterSay("Yep. She's for real?",0,0); SummonTimer = 4000; Phase++; break;
                            case 2: Thug2->MonsterSay("She wanted me to tell you that she appreciates the job that we did for her on the Furlbrows. Gave me a pile o'gold to split with you all.",0,0); SummonTimer = 7000; Phase++; break;
                            case 3: Thug3->MonsterSay(" See her face. It is really...",0,0); SummonTimer = 4000; Phase++; break;
                            case 4:
                            {
                                Thug1->SetFacingToObject(pPlayer);
                                Thug2->SetFacingToObject(pPlayer);
                                Thug3->SetFacingToObject(pPlayer);
                                Thug4->SetFacingToObject(pPlayer);
                                SummonTimer = 1000;
                                Phase++;
                            }break;
                            case 5: Thug1->MonsterSay("Whoa, what do we have here? Looks like we have ourselves an eavesdropper, boys.",0,0); SummonTimer = 4500; Phase++; break;
                            case 6: Thug1->MonsterSay("Only one thing to do with a louisy, good-for-nothin eavesdropper.",0,0); SummonTimer = 4500; Phase++; break;
                            case 7: Thug1->MonsterSay("DIE!!!",0,0); SummonTimer = 2000; Phase++; break;
                            ...
                            case 9:
                            {
                                Thug1->CombatStart(pPlayer,true);
                                Thug2->CombatStart(pPlayer,true);
                                Thug3->CombatStart(pPlayer,true);
                                Thug4->CombatStart(pPlayer,true);
                                SummonTimer = 5000;
                                Phase++;
                            }break;
                            case 10:
                            {
                                if(Player* pPlayer = me->GetPlayer(*me, PlayerGUID))
                                    pPlayer->KilledMonsterCredit(42417,PlayerGUID);
                                    SummonTimer = 2500;
                                    Phase++;                          
                            }break;
                            case 11:
                            {
                                pPlayer->CastSpell(pPlayer,79346,true);
                                me->MonsterTextEmote("Hurry back to the Furlbrow's Cottage",0,true);
                                if(pPlayer->GetQuestStatus(QUEST_LOUS_PARTING_THOUGHTS) == QUEST_STATUS_COMPLETE)
                                {
                                    if(Creature* pTwo = me->FindNearestCreature(42405,200.0f,true))
                                    {
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_horatioAI (pCreature);
    }

    struct npc_horatioAI : public ScriptedAI
    {
        npc_horatioAI(Creature *c) : ScriptedAI(c) {}
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
                ...
                if (pWho->IsWithinDistInMap(me, 25.0f) && !bSummoned)
                {                    
                    PlayerGUID = pWho->GetGUID();
                    StartSummon();
                }
                ...
                if(bSummoned)
                {
                    ...
                    if(Creature* Investigator02 = me->GetCreature(*me, Investigator02GUID))		
                    {
                        switch(Phase)
                        {
                            ...
                            case 2: Investigator02->MonsterSay("Damn shame what they did to Old Blanchy...",0,0); TextTimer = 6000; Phase++; break;
                            case 3:
                            {
                            ...
                            case 4: me->MonsterSay("Looks like they really put the cart.",0,0); TextTimer = 5000; Phase++; break;
                            case 5: me->MonsterSay("...before the horse.",0,0); TextTimer = 4000; Phase++; break;
                            case 6:
                            {
                            ...
                            case 7: me->SetFacingToObject(pPlayer); TextTimer = 60000; Phase++; break;
                            case 8: Reset(); break;
                            default: break;
                        }
                    }
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        if (pPlayer->GetQuestStatus(26209) == QUEST_STATUS_INCOMPLETE)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_HELLO_DRIFTER1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    
        if (pPlayer->GetQuestStatus(26209) == QUEST_STATUS_INCOMPLETE)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_HELLO_DRIFTER2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
        {
            ...
            {
            case 0:
                pCreature->MonsterSay("Listen, pal. I don't want any trouble, ok? I didn't see who murdered 'em, but I sure heard it! Lots of yelling. Human voices... you dig? Now get out of here before I change my mind about beating you up and takin' your shoes.",0,0);
                pPlayer->KilledMonsterCredit(CREDIT_SAY1,0);
                pCreature->SetStandState(UNIT_STAND_STATE_STAND);
                pCreature->ForcedDespawn(5000);
                break;
            case 1:
                pCreature->MonsterSay("I didn't see who killed 'm, bub/sis, but I got a whiff. Smelled rich, kinda like you. Damn shame too. Furlbrows were a fixture around here. Nice people, always willin' to share a meal or a patch of dirt.",0,0);
                pPlayer->KilledMonsterCredit(CREDIT_SAY2,0);
                pCreature->SetStandState(UNIT_STAND_STATE_STAND);
                pCreature->ForcedDespawn(5000);
                break;
            case 2:
                pCreature->MonsterSay("Who killed the Furlbrows? I'll tell you who killed the Furlbrows: KING VARIAN WRYNN. THAT'S WHO! And he's killin' the rest of us too. One bum at a time. The only thing I can tell you is that I saw some gnolls leavin' the place a few hours before the law arrived.",0,0);
                pPlayer->KilledMonsterCredit(CREDIT_SAY3,0);
                pCreature->SetStandState(UNIT_STAND_STATE_STAND);
                pCreature->ForcedDespawn(5000);
                break;
            case 3:
                pCreature->MonsterSay("Between you, me, and the tree, murlocs killed the Furlbrows. Yep, saw 'em with my own two eyes. Think they'd been casin' the joint for days, maybe months. They left in a hurry once they got wind of 'Johnny Law' and the idiot brigade over there...",0,0);
                pPlayer->KilledMonsterCredit(CREDIT_SAY4,0);
                pCreature->SetStandState(UNIT_STAND_STATE_STAND);
                pCreature->ForcedDespawn(5000);
                break;
            ...
            else
            {
                ...
                {    
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        pCreature->MonsterSay("I wonder if it's possible to eat rocks? Got plenty of rocks around here. Just imagine it! I'd be the richest person in the world for making that discovery!",0,0);
                        pCreature->SetReactState(REACT_AGGRESSIVE);
                        pCreature->AI()->AttackStart(pPlayer);
                        break;
                    case 5:
                        pCreature->MonsterSay("Looks like I found us a savory and clean piece of dirt! Tonight we eat like kings, Mr. Penguin! Of course I'll share it with you! You're my best friend!",0,0);
                        pCreature->SetReactState(REACT_AGGRESSIVE);
                        pCreature->AI()->AttackStart(pPlayer);
                        break;
                    case 6:
                        pCreature->MonsterSay("HAHAHAH! Good one, Mr. Penguin! GOOD ONE!",0,0);
                        pCreature->SetReactState(REACT_AGGRESSIVE);
                        pCreature->AI()->AttackStart(pPlayer);
                        break;
                    case 7:
                        pCreature->MonsterSay("What happened to me? I used to be the king of Stormwind!",0,0);
                        pCreature->SetReactState(REACT_AGGRESSIVE);
                        pCreature->AI()->AttackStart(pPlayer);
                        break;
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
          return new npc_crate_mineAI (pCreature);
    }
 
    struct npc_crate_mineAI  : public ScriptedAI
    {
        npc_crate_mineAI(Creature *c) : ScriptedAI(c) {}
        ...
        void PartySummoned()
        {
            if(!bSummoned)
            {
                if(Creature* pGlubtok1 = me->SummonCreature(42492,-9848.11f, 1395.29f, 37.70f, 0.56f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 85000)) 
                if(Creature* pShadowy1 = me->SummonCreature(42515, -9826.083f, 1406.738f, 36.885f, 3.56f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 85000))
                {        
                    GlubtoklGUID = pGlubtok1->GetGUID();
                    Shadowy1GUID = pShadowy1->GetGUID();
        ...
        void UpdateAI(const uint32 diff)
        {
            if(EntryTime <= diff)
            {
                ...
                {
                    //if(Player* pPlayer = me->GetPlayer(*me, PlayerGUID))
                    if(Creature* pGlubtok1 = me->GetCreature(*me,GlubtoklGUID))
                    if(Creature* pShadowy1 = me->GetCreature(*me,Shadowy1GUID))
    
                    {
                        switch(Phase)
                        {
                            case 0: EntryTime = 2500; Phase++; break;
                            case 1: pShadowy1->AddUnitMovementFlag(MOVEMENTFLAG_WALKING); pShadowy1->GetMotionMaster()->MovePoint(0, -9840.526f, 1399.37f, 37.177f); EntryTime = 7000; Phase++; break;
                            case 2: pGlubtok1->MonsterSay("What little human want? Why you call Glubtok?",0,0); EntryTime = 5000; Phase++; break;
                            case 3: pShadowy1->MonsterSay("Sad... Is this the life that you had hoped for, Glubtok? Running two bit extortion operations out of a cave?",0,0); EntryTime = 8500; Phase++; break;
                            case 4: pGlubtok1->MonsterSay("Glubtok crash you!",0,0); EntryTime = 4000; Phase++; break;
                            case 5: pShadowy1->MonsterSay("Oh will you? Do you dare cross that line and risk your life?",0,0); EntryTime = 6500; Phase++; break;
                            case 6: pShadowy1->MonsterSay("You may attempt to kill me - and fail - or you make take option two.",0,0); EntryTime = 6500; Phase++; break;
                            case 7: pGlubtok1->MonsterSay("What is option two.",0,0); EntryTime = 4000; Phase++; break;
                            case 8: pShadowy1->MonsterSay("You join me and I shower wealth and power upon you.",0,0); EntryTime = 7000; Phase++; break;
                            case 9: pGlubtok1->MonsterSay("So Glubtok have two choices die or be rich and powerful?",0,0); EntryTime = 7000; Phase++; break;
                            case 10: pGlubtok1->MonsterSay("Glubtok take choice two.",0,0); EntryTime = 6000; Phase++; break;
                            case 11: pShadowy1->MonsterSay("I thought you'd see it my way.",0,0); EntryTime = 6000; Phase++; break;
                            case 12: pShadowy1->MonsterSay("I will call you when the dawning is upon us.",0,0); EntryTime = 6000; Phase++; break;
                            case 13:
                            {      
                                me->ForcedDespawn(5000);
                                pGlubtok1->CastSpell(pGlubtok1,64446,true);
                                pGlubtok1->ForcedDespawn(1000);
                                pShadowy1->CastSpell(pShadowy1,64446,true);
                                pShadowy1->ForcedDespawn(1000);
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
          return new npc_homeless_citizenAI (pCreature);
    }
 
    struct npc_homeless_citizenAI  : public ScriptedAI
    {
        npc_homeless_citizenAI(Creature *c) : ScriptedAI(c) {}
        ...
        void UpdateAI(const uint32 diff)
        {
            if(HungryTimer < diff)
            {
                if(Creature *pStew = me->FindNearestCreature(NPC_STEW,10.0f,true))
                {
                    ...
                    switch(Phase)
                    {
                        case 0: me->RemoveStandFlags(UNIT_STAND_STATE_SLEEP); me->SetStandFlags(UNIT_STAND_STATE_STAND); HungryTimer = 1000; Phase++; break;
                        case 1: MoveInFront(me, pStew); HungryTimer = 1500; Phase++; break;
                        case 2: Eat(); HungryTimer = 2000; Phase++; break;
                        case 3:
                        {
                            if(Unit* pPlayer = me->GetPlayer(*pStew, pStew->GetUInt64Value(UNIT_FIELD_SUMMONEDBY)))
                                pPlayer->ToPlayer()->KilledMonsterCredit(42617, NULL);
                                HungryTimer = 25000;
                                Phase++; 
                                break;
                        }
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
          return new npc_shadowy_triggerAI (pCreature);
    }
 
    struct npc_shadowy_triggerAI  : public ScriptedAI
    {
        npc_shadowy_triggerAI(Creature *c) : ScriptedAI(c) {}
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
            ...
            if(pWho->ToPlayer()->GetQuestStatus(QUEST_THE_DAWNING_OF_NEW_DAY) == QUEST_STATUS_INCOMPLETE)
            {
                if (pWho->IsWithinDistInMap(me, 20.0f) && !bSummoned)
                {
                    PlayerGUID = pWho->GetGUID();
                    StartSpeech();   
                }
            ...
            if(SummonTimer < diff)
            {
                if(bSummoned)
                {
                    ...
                    if(Creature* Shadowy2 = me->GetCreature(*me, Shadowy2GUID))
                    {
                        switch(Phase)
                        {
                            case 0: me->MonsterTextEmote("The rally is about to begin!", NULL, true); SummonTimer = 1500; Phase++; break;
                            case 1: Shadowy2->MonsterYell("Gather, brothers and sisters! Come, all, and listen!",0,0); SummonTimer = 3000; Phase++; break;
                            case 2: Shadowy2->MonsterYell("Brothers. Sisters. We are ABANDONED - the orphaned children of Stormwind.",0,0); SummonTimer = 5500; Phase++; break;
                            case 3: Shadowy2->MonsterYell("Our 'king' sits atop his throne made of gold and shrugs at our plight!",0,0); SummonTimer = 4000; Phase++; break;
                            case 4: Shadowy2->MonsterYell("Meanwhile, our children die of starvation on these very streets!",0,0); SummonTimer = 4500; Phase++; break;
                            case 5: Shadowy2->MonsterYell("HIS war, not ours, cost us our livelihood. WE paid for the Alliance's victories with our blood and the blood of our loved ones!",0,0); SummonTimer = 4500; Phase++; break;
                            case 6: Shadowy2->MonsterYell("The time has come, brothers and sisters, to stop this injustice!",0,0); SummonTimer = 4500; Phase++; break;
                            case 7: Shadowy2->MonsterYell("The government of Stormwind, of the ALLIANCE, must be made accountable for what it has done to us!",0,0); SummonTimer = 4500; Phase++; break;
                            case 8: Shadowy2->MonsterYell("Today, we are reborn! Today, we take a stand as men and women, not nameless, faceless numbers!",0,0); SummonTimer = 4500; Phase++; break;
                            case 9:
                            {
                                if(Creature* pListener = me->FindNearestCreature(NPC_LISTENER,35.0f,true))
                                {
                                    if(!bText)
                                    {
                                        me->MonsterTextEmote("<homeless people applaud and cheer>", NULL, true);
                                        pListener->TextEmote(TEXT_EMOTE_APPLAUD,PlayerGUID,false);
                                        bText = true;
                                    }
                                    pListener->HandleEmoteCommand(EMOTE_ONESHOT_APPLAUD);
                                    pPlayer->KilledMonsterCredit(42680,PlayerGUID);
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
          return new npc_shadowy_towerAI (pCreature);
    }
 
    struct npc_shadowy_towerAI  : public ScriptedAI
    {
        npc_shadowy_towerAI(Creature *c) : ScriptedAI(c) {}
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
            ...
            if (pWho->GetTypeId() != TYPEID_PLAYER)
                return;
            //if (pWho->HasAura(79528))
              //  return true;
 
            if(pWho->ToPlayer()->GetQuestStatus(QUEST_SECRETS_OF_THE_TOWER) == QUEST_STATUS_INCOMPLETE)
            {
                if (pWho->IsWithinDistInMap(me, 10.0f) && !bSumm)
                {
                    PlayerGUID = pWho->GetGUID();
                    StartEvent();   
                }
            ...
            if(SummonTimer < diff)
            {
                if(bSumm)
                {
                    ...
                    if(Creature* Shadowy3 = me->GetCreature(*me, Shadowy3GUID))
                    {
                        switch(Phase)
                        {
                            case 0: Glubtok3->SetInFront(me); SummonTimer = 1000; Phase++; break;
                            case 1: Glubtok3->MonsterSay("The gnolls have failed, mistress.",0,0); SummonTimer = 6000; Phase++; break;
                            case 2: Shadowy3->MonsterSay("They provided the distraction I required. We continue as planned.",0,0); SummonTimer = 6000; Phase++; break;
                            case 3: Glubtok3->MonsterSay("But mistress, the admiral is sti...",0,0); SummonTimer = 6000; Phase++; break;
                            case 4: Shadowy3->MonsterSay("We will free the admiral during the dawning.",0,0); SummonTimer = 6000; Phase++; break;
                            case 5: Glubtok3->MonsterSay("Yes, mistress.",0,0); SummonTimer = 6000; Phase++; break;
                            case 6: Shadowy3->MonsterSay("Judgment day is soon upon us, Helix.",0,0); SummonTimer = 6000; Phase++; break;
                            case 7: Shadowy3->MonsterSay("Call for the people. I wish to speak to them one last time before the dawning.",0,0); SummonTimer = 6000; Phase++; break;
                            case 8: Glubtok3->MonsterSay("Moonbrook, mistress?",0,0); SummonTimer = 6000; Phase++; break;
                            case 9: Shadowy3->MonsterSay("Aye. Tonight.",0,0); SummonTimer = 2000; Phase++; break;
                            case 10: pPlayer->CastSpell(pPlayer,79534,true); SummonTimer = 1000; Phase++; break;
                            case 11: 
                            {
                                if(!bExit)
                                {
...
public:
    ...
    bool OnQuestAccept(Player* plyr, Creature* pc, Quest const *pQuest)
    {
        if (pQuest->GetQuestId() == QUEST_RISE_OF_THE_BROTHERHOOD)
        {
            if(!(CAST_AI(npc_rise_br::npc_rise_brAI, pc->AI())->EventStarted))
            {
                    CAST_AI(npc_rise_br::npc_rise_brAI, pc->AI())->EventStarted = true;
                    CAST_AI(npc_rise_br::npc_rise_brAI, pc->AI())->PlayerGUID = plyr->GetGUID(); 
            }
        }return true;
    }
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
          return new npc_rise_brAI (pCreature);
    }
 
    struct npc_rise_brAI  : public ScriptedAI
    {
        npc_rise_brAI(Creature *c) : ScriptedAI(c) {}
                ...
                if(EventTimer < diff)
                {
                    ...
                    if(Creature* Investigator2 = me->FindNearestCreature(NPC_INVESTIGATOR3,50.0f,true))
                    {
                        switch(Phase)
                        {
                            ...
                            case 2: Hope->MonsterSay("You bastards will burn for what you did.",0,0); EventTimer = 4000; Phase++; break;
                            case 3: me->MonsterSay("Hope! What...",0,0); EventTimer = 1500; Phase++; break;
                            case 4: Hope->MonsterSay("Hope? Is that what I was supposed fell when I saw my father decapitated by your henchmen?",0,0); EventTimer = 12000; Phase++; break;
                            case 5: Hope->MonsterSay("Hope is a cruel joke, played upon us by a harsh and uncaring world. There is no Hope, there is only Vanessa, Vanessa VanCleef.",0,0); EventTimer = 9500; Phase++; break;
                            case 6: Hope->CastSpell(Hope,SPELL_TRANSFORM_VANEESA,true); EventTimer = 2500; Phase++; break;
                            case 7: 
                            {
                            ...
                            case 8: Hope->MonsterSay("Tie them up.",0,0); EventTimer = 1500; Phase++; break;
                            case 9:
                            {
                                DoCast(me,79724);
                                DoStun();
                            ...
                            case 10: Hope->GetMotionMaster()->MovePoint(1, -10512.36f, 1044.36f, 60.518f); EventTimer = 3000; Phase++; break;
                            case 11: Hope->CastSpell(Hope,SPELL_HIT_ME,true); EventTimer = 1500; Phase++; break;
                            case 12: 
                            {
                            ...
                            case 15: Ripsnarl->GetMotionMaster()->MovePoint(3, -10511.34f, 1042.46f, 60.5172f); EventTimer = 1500; Phase++; break;
                            case 16:
                            {   
                                Hope->SetFacingToObject(pPlayer);
                            ...
                            case 20: Hope->MonsterYell("Leave nothing but ashes in your wake, brothers! Burn Sentinel Hill in the ground!",0,0); EventTimer = 4000; Phase++; break;
                            case 21: 
                            {
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_defias_blackguardAI(pCreature);
    }

    struct npc_defias_blackguardAI : public ScriptedAI
    {
        npc_defias_blackguardAI(Creature *c) : ScriptedAI(c) {}
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_fire_triggerAI(pCreature);
    }

    struct npc_fire_triggerAI : public ScriptedAI
    {
        npc_fire_triggerAI(Creature *c) : ScriptedAI(c) {}
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_summonerAI(pCreature);
    }

    struct npc_summonerAI : public ScriptedAI
    {
        npc_summonerAI(Creature *c) : ScriptedAI(c) {}
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_horatio_investigateAI (pCreature);
    }

    struct npc_horatio_investigateAI : public ScriptedAI
    {
        npc_horatio_investigateAI(Creature *c) : ScriptedAI(c) {}
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
                ...
                if (pWho->IsWithinDistInMap(me, 15.0f))
                {       
                    StartSummon();            
                    PlayerGUID = pWho->GetGUID();
                }
                ...
                if(bSummonn)
                {
                    ...
                    if(Creature* Home = me->GetCreature(*me, HomeGUID))	
                    {
                        switch(Phase)
                        {
                            ...
                            case 2: Investigator04->MonsterSay("Natural causes? Two bullets in the chest and his shoes are on his head. What kind of natural death would that be?",0,0); TextTimer = 6000; Phase++; break;
                            case 3:
                            {
                            ...
                            case 4: me->MonsterSay("This was an execution. Whoever did this was sending a message,,,",0,0); TextTimer = 5000; Phase++; break;
                            case 5: me->MonsterSay("A message for anyone that would dare snitch on these cryminals.",0,0); TextTimer = 4000; Phase++; break;
                            case 6: me->MonsterSay("It would appear that poor Lou really put this foot...",0,0); TextTimer = 4000; Phase++; break;
                            case 7: me->MonsterSay("In his mouth...",0,0); TextTimer = 30000; Phase++; break;
                            case 8: Reset(); break;
                            default: break;
                        }
                    }
