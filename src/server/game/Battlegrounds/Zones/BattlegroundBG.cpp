...
BattlegroundBG::BattlegroundBG()
{
    ...
    m_BgCreatures.resize(BG_BG_ALL_NODES_COUNT + 3);//+3 for aura triggers
...
void BattlegroundBG::EventPlayerClickedOnFlag(Player *source, GameObject* /*target_obj*/)
{
    ...
    GameObject* obj=GetBgMap()->GetGameObject(m_BgObjects[node*8+3]);
...
bool BattlegroundBG::SetupBattleground()
{
    ...
    {
        if (!AddObject(BG_BG_OBJECT_BANNER_NEUTRAL + 8*i, NodeID[i], BG_BG_NodePositions[i][0], BG_BG_NodePositions[i][1], BG_BG_NodePositions[i][2], BG_BG_NodePositions[i][3], 0, 0, sin(BG_BG_NodePositions[i][3]/2), cos(BG_BG_NodePositions[i][3]/2), RESPAWN_ONE_DAY)
...
WorldSafeLocsEntry const* BattlegroundBG::GetClosestGraveYard(Player* player)
{
    ...
    if (!nodes.empty())
    {
        ...
        float mindist = 999999.0f; //some kind of hack ;/
